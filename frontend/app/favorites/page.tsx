...
export default function FavoritesScreen() {
  ...
  const {
    ...
    () =>
      Promise.all(
        favorites.map((id) => api.get<PartnerDetail>(`/partners/${id}`)),
      ),
  );
  ...
  return (
    <div
      className="buyer-secondary-page buyer-favorites-page screen-scroll-with-tabbar"
      style={{
        width: "100%",
        background: t.bg,
        fontFamily: fontFn,
    ...
    >
      ...
      {favorites.length === 0 ? (
        <div
          className="app-readable-content buyer-empty-wrap"
          style={{
            padding: "16px",
            display: "flex",
            flexDirection: "column",
        ...
        <div
          className="app-readable-content buyer-favorites-loading"
          style={{
            padding: "14px 16px",
            display: "flex",
            flexDirection: "column",
        ...
        <div
          className="app-content buyer-favorites-grid"
          style={{
            padding: "14px 16px",
            display: "flex",
            flexDirection: "column",
        ...
        >
          <div
            className="buyer-section-title buyer-favorites-title"
            style={{
              fontSize: 17,
              fontWeight: 700,
              marginBottom: 12,
          ...
          {stores.map(({ partner, offers }) => {
            const activeOffers = offers.filter(
              (offer) => offer.availability === "IN_STOCK" && offer.stock > 0,
            );
            const minPrice = activeOffers.length
              ? Math.min(
                  ...activeOffers.map((offer) =>
                    Number(offer.price ?? offer.new_price),
                  ),
                )
              : null;
            return (
                  ...
                  >
                    {activeOffers.length > 0 ? (
                      <Badge tone="green" size="sm">
                        {formatPositionsCount(activeOffers.length)} от{" "}
                        {minPrice} ₸
                      </Badge>
                    ) : (
